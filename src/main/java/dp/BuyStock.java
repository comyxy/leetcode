 ...
 * 对状态与选择进行枚举 选择：买入、卖出、无操作 状态：3个，天数、至多交易次数、股票持有状态（0未持有，1持有） dp[i][k][0 or 1] 0<=i<=n-1, 1<=k<=K n为天数
 * K最多交易数 共 n*K*2 个状态 状态转移： dp[i][k][0] = max(dp[i-1][k][0], dp[i-1][k][1]+prices[i]) = max(选择为无操作,
 * 选择为卖出股票) 今天我没有持有股票，有两种可能：要么是我昨天就没有持有，然后今天选择无操作，所以我今天还是没有持有； 要么是我昨天持有股票，但是今天我卖出股票了，所以我今天没有持有股票了。
 *
 * <p>dp[i][k][1] = max(dp[i-1][k][1], dp[i-1][k-1][0]-prices[i]) = max(选择为无操作， 选择为购入股票)
 * 今天我持有着股票，有两种可能：要么我昨天就持有着股票，然后今天选择无操作，所以我今天还持有着股票； 要么我昨天本没有持有，但今天我选择购入，所以今天我就持有股票了。 基础条件：
 */
public class BuyStock {
  ...
  /**
   * LeetCode123 K=2 时 dp[i][k][0] = max(dp[i-1][k][0], dp[i-1][k][1]+prices[i]) dp[i][k][1] =
   * max(dp[i-1][k][1], dp[i-1][k-1][0]-prices[i]) ==> dp[i][2][0] = max(dp[i-1][2][0],
   * dp[i-1][2][1]+prices[i]); dp[i][2][1] = max(dp[i-1][2][1], dp[i-1][1][0]-prices[i]);
   * dp[i][1][0] = max(dp[i-1][1][0], dp[i-1][1][1]+prices[i]); dp[i][1][1] = max(dp[i-1][1][1],
   * dp[i-1][0][0]-prices[i]);
   */
  ...
  /**
   * LeetCode188 dp[i][k][0] = max(dp[i-1][k][0], dp[i-1][k][1]+prices[i]) dp[i][k][1] =
  ...
  public int maxProfit5(int coolDown, int[] prices) {
    ...
    Queue<Integer> cache_dp_ik0 = new LinkedList<>();
